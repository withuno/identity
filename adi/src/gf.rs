//
// Copyright 2021 WithUno, Inc.
// SPDX-License-Identifier: AGPL-3.0-only
//

// Referenced implementation:
// https://github.com/hashicorp/vault/blob/v1.6.2/shamir/shamir.go

use rand::{thread_rng, Rng};

use subtle::ConditionallySelectable;
use subtle::ConstantTimeEq;

#[derive(Debug)]
pub struct Polynomial {
    pub coefficients: Vec<u8>,
}

pub fn make_polynomial(intercept: u8, degree: usize) -> Polynomial {
    let mut p = Polynomial {
        //coefficients: Vec::with_capacity(degree + 1),
        coefficients: vec![0u8; degree + 1],
    };

    p.coefficients[0] = intercept;

    thread_rng().fill(&mut p.coefficients[1..]);

    p
}

pub fn interpolate_polynomial(x_samples: &[u8], y_samples: &[u8], x: u8) -> u8 {
    let limit = x_samples.len();
    let mut result = 0u8;
    let mut basis;

    for i in 0..limit {
        basis = 1;
        for j in 0..limit {
            if i == j {
                continue;
            }

            let num = add(x, x_samples[j]);
            let denom = add(x_samples[i], x_samples[j]);
            let term = div(num, denom);
            basis = mult(basis, term);
        }

        let group = mult(y_samples[i], basis);
        result = add(result, group);
    }

    result
}

impl Polynomial {
    pub fn evaluate(&self, x: u8) -> u8 {
        if x == 0 {
            return self.coefficients[0];
        }

        let degree = self.coefficients.len() - 1;
        let mut out = self.coefficients[degree];

        let mut i = degree - 1;
        loop {
            let coeff = self.coefficients[i];
            out = add(mult(out, x), coeff);

            if i == 0 {
                break;
            }

            i = i - 1;
        }

        out
    }
}

pub fn add(a: u8, b: u8) -> u8 {
    a ^ b
}

pub fn div(a: u8, b: u8) -> u8 {
    if b == 0 {
        panic!("divide by zero");
    }

    let log_a: u8 = LOG_TABLE[a as usize];
    let log_b: u8 = LOG_TABLE[b as usize];

    let diff = (((log_a as i16) - (log_b as i16)) + 255) % 255;

    let ret = EXP_TABLE[diff as usize];

    // ret = subtle.ConstantTimeSelect(subtle.ConstantTimeByteEq(a, 0), 0, ret)
    u8::conditional_select(&ret, &0, a.ct_eq(&0))
}

// do i need to take/return a subtle::Choice here?
// how do i avoid using `as usize` and `as i16`
pub fn mult(a: u8, b: u8) -> u8 {
    let log_a: u8 = LOG_TABLE[a as usize];
    let log_b: u8 = LOG_TABLE[b as usize];

    let sum = ((log_a as i16) + (log_b as i16)) % 255;

    let ret = EXP_TABLE[sum as usize];

    //ret = subtle.ConstantTimeSelect(subtle.ConstantTimeByteEq(a, 0), 0, ret)
    //ret = subtle.ConstantTimeSelect(subtle.ConstantTimeByteEq(b, 0), 0, ret)
    let ret = u8::conditional_select(&ret, &0, a.ct_eq(&0));
    u8::conditional_select(&ret, &0, b.ct_eq(&0))
}

// https://github.com/hashicorp/vault/blob/master/shamir/tables.go
pub const LOG_TABLE: [u8; 256] = [
    ,
];

pub const EXP_TABLE: [u8; 256] = [
    ,
];



#[cfg(test)]
mod tests {
    use super::*;

    #[test]
    fn field_add() {
        assert_eq!(add(16, 16), 0);
        assert_eq!(add(3, 4), 7);
    }

    #[test]
    fn field_mult() {
        assert_eq!(mult(3, 7), 9);
        assert_eq!(mult(3, 0), 0);
        assert_eq!(mult(0, 3), 0);
    }

    #[test]
    #[should_panic]
    fn field_divide_by_zero() {
        div(7, 0);
    }

    #[test]
    fn field_divide() {
        assert_eq!(div(0, 7), 0);
        assert_eq!(div(3, 3), 1);
        assert_eq!(div(6, 3), 2);
    }

    #[test]
    fn polynomial_random() {
        let p = make_polynomial(42, 2);
        assert_eq!(p.coefficients[0], 42);
    }

    #[test]
    fn polynomial_eval() {
        let p = make_polynomial(42, 1);

        assert_eq!(p.evaluate(0), 42);
        assert_eq!(p.evaluate(1), add(42, mult(1, p.coefficients[1])))
    }

    #[test]
    fn polynomial_interpolate_rand() {
        let v: Vec<u8> = (0..=255).collect();

        for i in v {
            let p = make_polynomial(i, 2);

            let x_vals: Vec<u8> = vec![1, 2, 3];
            let y_vals: Vec<u8> =
                vec![p.evaluate(1), p.evaluate(2), p.evaluate(3)];

            let out = interpolate_polynomial(&x_vals, &y_vals, 0);
            assert_eq!(out, i);
        }
    }

    #[test]
    fn test_tables() {
        for i in 1..256 {
            let log_v: u8 = LOG_TABLE[i];
            let exp_v: u8 = EXP_TABLE[log_v as usize];

            assert_eq!(exp_v, i as u8);
        }
    }
}
