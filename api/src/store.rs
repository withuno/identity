...
pub trait Database: Send + Sync + Clone + Debug {
    ...
    async fn get_version<P, Q>(&self, version: Q, object: P)
    -> Result<Vec<u8>>
    where
        P: AsRef<Path> + Send,
        Q: AsRef<Path> + Send,
    ...
    async fn put_version<P, Q>(&self, version: Q, object: P, data: &[u8])
    -> Result<()>
    where
        P: AsRef<Path> + Send,
        Q: AsRef<Path> + Send,
    ...
    async fn del_version<P, Q>(&self, version: Q, object: P)
    -> Result<()>
    where
        P: AsRef<Path> + Send,
        Q: AsRef<Path> + Send,
